...
async function addWatermark(pdfBuffer) {
    try {
        ...
        console.error('Błąd podczas edycji PDF:', error);
        throw error;
    }
}
...
(async () => {
    ...
    } catch (e) { console.error('Błąd rejestracji komend:', e); }
})();
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'konw') {
        ...
        for (const att of filesToProcess) {
            ...
            } catch (e) { console.error(`Błąd pliku ${att.name}:`, e); }
        }

        if (processedFiles.length === 0) {
            return interaction.editReply('❌ Nie udało się przetworzyć żadnego pliku PDF.');
        }
        ...
        try {
            ...
            await interaction.editReply('❌ Bot nie ma uprawnień do wysyłania wiadomości na tym kanale.');
        }
    }
});
...
client.on('messageCreate', async message => {
    ...
    if (attachments.length === 0) return message.reply('❌ Załącz co najmniej jeden plik PDF!');
    ...
    if (processedFiles.length > 0) {
        await message.channel.send({ content: `✅ Przetworzono ${processedFiles.length} plików dla: ${message.author.tag}`, files: processedFiles });
    }
